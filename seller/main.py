...
Env vars:
  ...
  SELLER_PRIVATE_KEY — optional, used for seller receipt attestation signing
...
# Load repo-root .env (single source of truth) before reading any env vars.
bootstrap_env()

app = FastAPI(title="SLAgent-402 LLM Seller", version="0.3.0")
...
FALLBACK_VALID_INVOICE = {
    ...
    "notes": "Generated by SLAgent-402 demo seller (fallback)",
}
...
def _llm_headers(*, provider: str, model: str, used: bool, mode: str) -> dict[str, str]:
    """Attach demo-friendly evidence headers (no sensitive content)."""
    return {
        "X-LLM-Provider": provider,
        "X-LLM-Model": model,
        "X-LLM-Used": "true" if used else "false",
        "X-LLM-Mode": mode,
    }
...
async def capabilities() -> JSONResponse:
    ...
    return JSONResponse(content={
        ...
        "llm_available": bool(has_key and not fallback),
        "supported_schemas": ["invoice_v1"],
        "supported_modes": ["fast", "slow", "invalid", "error", "timeout"],
        "endpoints": {
            "call": "POST /seller/call?mode=fast|slow|invalid|error|timeout&delay_ms=0..8000",
...
async def seller_call(
    ...
    if mode == "error":
        return JSONResponse(
            status_code=500,
            content={"error": "Simulated upstream error"},
            headers=_llm_headers(provider="deterministic", model="simulator", used=False, mode=mode),
        )

    if mode == "timeout":
        # Gateway httpx timeout is 30s; exceed it so gateway raises upstream timeout.
        await asyncio.sleep(35.0)
        return JSONResponse(
            content={"error": "Should have timed out"},
            headers=_llm_headers(provider="deterministic", model="simulator", used=False, mode=mode),
        )
...
async def _fallback_response(mode: str) -> JSONResponse:
    ...
    if mode == "fast":
        ...
        return JSONResponse(
            ...
            headers=_llm_headers(provider="deterministic", model="fallback", used=False, mode=mode),
        )
    elif mode == "slow":
        ...
        return JSONResponse(
            content=FALLBACK_INVALID_RESPONSE,
            headers=_llm_headers(provider="deterministic", model="fallback", used=False, mode=mode),
        )
...
async def _llm_valid_response(client: GeminiClient) -> JSONResponse:
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            return JSONResponse(
                content=data,
                headers=_llm_headers(provider="google-gemini", model=client.model, used=True, mode=mode),
            )
...
async def _llm_invalid_response(client: GeminiClient) -> JSONResponse:
    ...
    try:
        ...
        return JSONResponse(
            content=data,
            headers=_llm_headers(provider="google-gemini", model=client.model, used=True, mode="invalid"),
        )
    except (GeminiError, JSONExtractionError):
        return JSONResponse(
            content=FALLBACK_INVALID_RESPONSE,
            headers=_llm_headers(provider="deterministic", model="fallback", used=False, mode="invalid"),
        )
